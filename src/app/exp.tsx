  ...
  const projects = [
   {
      ...
      languages: ["TypeScript", "Tailwind CSS", "React.js", "JavaScript", "Vite", "Swiper", "ESLint", "ts-node", "sucrase"],
      progress: 100
    },
   {
      title: "iClínica",
    ...
    >
      ...
      <div className="grid gap-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {projects.map((project, index) => (
          <div key={index} className="relative w-full h-96 [perspective:1000px] group overflow-hidden">
            <div className="relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
              
              {/* Frente do card: carrossel */}
              <div className="absolute w-full h-full backface-hidden">
                <ClientCarousel media={project.media} />
              </div>

              {/* Verso do card: descrição */}
              <div className="absolute w-full h-full bg-[#012E40] text-white p-6 rounded-xl backface-hidden [transform:rotateY(180deg)]">
                <h3 className="text-xl font-bold mb-2 text-center">{project.title}</h3>
                <p className="text-[0.8rem] text-gray-300 mb-4">{project.description}</p>
